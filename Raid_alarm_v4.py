...
def week_on_ao():
    '''
    Возвращает boolen:
    True последовательность событий имеет вид -  "Спорные земли" -> "Праховей".
    False последовательность событий имеет вид - "Праховей" -> "Спорные земли".
    '''
    week = gmt_time().isocalendar()[1]
    weekday = gmt_time().isocalendar()[2]
    tmp_time = gmt_time().hour


    # Для чётных недель
    if (week % 2) == 0:

        if weekday < 4: # Понедельник Вторник Среда
            return False  

        # NEED TEST
        if weekday == 4: # Для четверга 
            if tmp_time > 10: # До обновления клиента, база - 13-00мск
                return True
            else:
                return False
        
        if weekday > 4: # Для Пятница Суббота Воскресенье
            return True        
